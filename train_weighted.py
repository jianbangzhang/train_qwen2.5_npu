...
class WeightedLossTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        loss_fct = torch.nn.CrossEntropyLoss(ignore_index=IGNORE_TOKEN_ID, reduction="none")
        loss = loss_fct(logits.view(-1, logits.size(-1)), labels.view(-1))

        loss = loss.view(-1) * weights.view(-1)
        valid_tokens = attention_mask.view(-1).sum()
        weighted_loss = loss.sum() / valid_tokens if valid_tokens > 0 else loss.sum()
...
def preprocess(
    ...
    for i, msg in enumerate(messages):
        ...
        if role in ["system", "user"]:
            ...
            weights.append([0] * len(tokenized["input_ids"]))
            ...
            weight_lst = [1] * len(input_id)
            if "Action:" in content:
                action_ids = tokenizer.encode("Action")
                action_index = None
                for i in range(len(input_id) - len(action_ids) + 1):
                    if input_id[i:i + len(action_ids)] == action_ids:
                        action_index = i
                        break
                if action_index is not None:
                    weight_lst = [1] * len(input_id[:action_index]) + [2] * len(input_id[action_index:])

            weights.append(weight_lst)
        else:
