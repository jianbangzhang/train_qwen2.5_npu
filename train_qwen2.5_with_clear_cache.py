...
"""
# File       : train_qwen2.5_with_clear_cache.py
# Time       ：2024/11/14 16:53
# Author     ：jianbang
# version    ：python 3.10
...
class CacheClearAfterSaveCallback(TrainerCallback):
    """Custom callback to clear cache after saving the model."""

    def on_save(self, args, state, control, model=None, tokenizer=None, **kwargs):
        """This will be called after saving the model."""
        # Check for NPU device
        if torch.npu.is_available():
            torch.npu.empty_cache()
            print(f"After saving model at step {state.global_step}: Clearing cache...")
        else:
            print("No supported device found for cache clearing.")
        return control
...
def train():
    ...
    trainer = Trainer(
        ...
        args=training_args,
        tokenizer=tokenizer,
        callbacks=[CacheClearAfterSaveCallback()],
        **data_module
    )
